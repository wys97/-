...
import {Button, DatePicker, Field, Form, Grid, Input, Message, Radio, Select} from '@alifd/next';
...
export default class LoansUpdateManage extends Component {

  field = new Field(this);
  static displayName = 'LoansUpdateManage';
  static propTypes = {};
  static defaultProps = {};
    ...
    loansManageApi.projectStatus()
      .then((res) => {
        if (res.data.code === '200') {
          this.state.projectStatus = res.data.data;
        } else {
  ...
  getprojectTerm = () => {    //期限单位-下拉
    loansManageApi.projectTermUnit()
      .then((res) => {
        if (res.data.code === '200') {
          this.state.projectTermUnit = res.data.data;
        } else {
          Message.error(res.data.message);
        }
      });
  ...
  getProjectDetail = () => {       //项目管理-详情
    loansManageApi.projectDetail(this.state.id)
      .then((res) => {
        if (res.data.code === '200') {
  ...
  projectUpdate = (v, e) => {
    ...
    loansManageApi.projectUpdate(v)
      .then((res) => {
        if (res.data.code === '200') {
          this.getProjectDetail();
        ...
        <IceContainer>
          <Form labelTextAlign={'right'}  {...formItemLayout} field={this.field}>
            <div className="CustomerTabTitle" style={{display: 'flex', justifyContent: 'space-between'}}>
              <h3 style={{marginTop: '-4px'}}>修改贷款项目</h3>
                ...
                <Row>
                  <Col span="12">
                    <FormItem style={styles.formItem} required requiredMessage="请填写项目编号" label="项目编号:">
                      <p>{this.field.getValue('projectNo')}</p>
                    </FormItem>
                  </Col>
                  <Col span="12">
                    <FormItem labelTextAlign='right' style={styles.formItem} label="项目状态:" required
                              requiredMessage="请选择项目状态">
                      <Select followTrigger name="projectStatus" style={styles.formInputBorder}>
                        {
                ...
                <Row>
                  ...
                  <Col span="12">
                    <FormItem style={styles.formItem} label="创建时间:">
                      <p>{this.field.getValue('creatorTime')}</p>
                    </FormItem>
                  </Col>
                </Row>
                <Row>
                  <Col span="12">
                    <FormItem style={styles.formItem} label="修改人员:">
                      <p>{this.field.getValue('modifyName')}</p>
                    </FormItem>
                  </Col>
                  <Col span="12">
            ...
            <Form.Submit validate type="primary" style={styles.saveButton} onClick={this.projectUpdate}>保存</Form.Submit>
          </Form>
        </IceContainer>
      </div>
    );
