...
import {Checkbox, Dialog, Field, Form, Grid, Input, Message} from '@alifd/next';
...
export default class RepaySetting extends Component {

  static displayName = 'RepaySetting';
  static propTypes = {};
  static defaultProps = {};
  ...
  getData = () => {
    productManageApi.rateSettingList(this.state.formValue.productNo).then((res) => {
  ...
  lineBtnFn = {
      ...
      productManageApi.rateSettingDetail(row.id).then((res) => {
        if (res.data.code === '200') {
          ...
          Object.keys(res.data.data).map((item) => {
            if (item.indexOf('interestRate') !== -1 && item.indexOf('Str') === -1 && res.data.data[item] && res.data.data[item] !== '0') {
              res.data.data.rates.push(item.replace(/[^0-9]/ig, ""));
            }
          });
    ...
    del: (val, index, row) => {
      Dialog.show({title: '操作', content: '确认删除该利率配置吗？', onOk: () => this.delFn(row)});
    }
  };
  ...
  addRates = (param) => {
    productManageApi.addRateSetting(param)
      .then((res) => {
        if (res.data.code === '200') {
          this.onClose(true);
          this.setState({
  ...
  editRates = (param) => {
    productManageApi.editRateSetting(param)
      .then((res) => {
        if (res.data.code === '200') {
          this.onClose(true);
          this.setState({
  ...
  render() {
    const loanMaxAmount = this.state.type === 'add' ?
      <FormItem labelTextAlign='right' name="pass" style={styles.formItem} required label="金额上限:">
    ...
    const loanMinAmount = this.state.type === 'add' ?
      <FormItem labelTextAlign='right' name="pass" style={styles.formItem} required label="金额下限:">
      ...
      <div>
        <IceContainer>
          <div className='contain-con'>
            <p style={{borderBottom: '1px solid #DDD', paddingBottom: '10px'}}>贷款利率配置</p>
        ...
        <Dialog
          ...
          <IceContainer>
            <h3 style={{
              ...
              paddingBottom: '15px',
            }}>{this.state.type === 'add' ? '新增' : '修改'}产品利率</h3>
            <div className='contain-con'>
              <Form labelTextAlign={'right'} {...formItemLayout} field={this.field}>
                <Row>
                  <Col span="12">
                    <FormItem labelTextAlign='right' style={styles.formItem} label="产品编号:" required
                  ...
                  <Col span="12">
                    <FormItem labelTextAlign='right' style={styles.formItem} label="产品名称:" required
                ...
                <Row>
                  <Col span="24">
                    <FormItem labelTextAlign='right' style={styles.formItem} required requiredMessage="支持期数是必选字段"
                              label="支持期数:">
                ...
                {this.state.rates.map((item, idx) => {
                  return <Row key={idx}>
                    <Col span="24">
                      <FormItem labelTextAlign='right' style={styles.formItem} required requiredMessage="月利率是必填字段"
                                label={item + '期月利率'}>
                        <Input name={'interestRate' + item} addonTextAfter="%"/>
                      </FormItem>
                    </Col>
                  </Row>
                })}
