import React, {Component} from 'react';
import DetailForm from '../../components/DetailForm';
import sysOperateLogApi from '../../../api/OperationManage/SysOperateLog';
import {Message} from "@alifd/next";

const col = [
  {label: '日志编号：', require: false, key: 'logId'},
  {label: '', require: false, key: ''},
  {label: '用户编号：', require: false, key: 'operatorId'},
  {label: '', require: false, key: ''},
  {label: '用户名：', require: false, key: 'loginName'},
  {label: '姓名', require: false, key: 'operatorName'},
  {label: '操作类型：', require: false, key: 'actionType'},
  {label: '', require: false, key: ''},
  {label: '客户端IP：', require: false, key: 'clientIp'},
  {label: '', require: false, key: ''},
  {label: '请求报文：', require: false, key: 'request', row: 2, line: true},
  {label: '返回报文：', require: false, key: 'response', row: 2, line: true},
  {label: '异常报文：', require: false, key: 'exception', row: 2, line: true},
  {label: '开始时间：', require: false, key: 'startTime'},
  {label: '结束时间：', require: false, key: 'endTime'}
];

export default class TaskLogDetail extends Component {
  static displayName = 'TaskLogDetail';

  static propTypes = {};

  static defaultProps = {};

  constructor(props) {
    super(props);
    this.state = {
      formValue: {},
      logId: this.props.location && this.props.location.state && this.props.location.state.name,
      title: '系统日志查询详情'
    }
  }

  componentWillMount() {
    if (this.props.location.state && this.props.location.state.name) {
      this.loadDetailInfo();
    } else {
      this.props.history.push('/myWorkspace/home');
    }
  }

  loadDetailInfo = () => {
    sysOperateLogApi.sysLogDetail(this.state.logId).then((res) => {
      if (res.data.code === '200') {
        this.setState({
          formValue: res.data.data
        })
      } else {
        Message.error(res.data.message);
      }
    });
  };

  render() {
    // 如果刷新浏览器, state将为undefined, 所以跳转回首页
    if (this.props.location.state === null || this.props.location.state === undefined) {
      this.props.history.push({pathname: '/'});
      return (<div/>);
    }
    return (
      <div>
        <DetailForm col={col} data={this.state.formValue} title={this.state && this.state.title}
                    history={this.props.history}/>
      </div>
    );
  }
}
