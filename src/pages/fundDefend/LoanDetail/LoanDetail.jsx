...
import loanDetailApi from '../../../api/FundDefund/LoanDetail';
...
export default class LoanDetail extends Component {
  ...
  getData = () => {
    ...
    loanDetailApi.checkPayList(params)
      .then((res) => {
        if (res.data.code === '200') {
          this.setState({
  ...
  lineBtn = [{
    ...
    permission: 'fund:check:pay:restart'
  },
    {
      name: '勾兑',
      type: 'blend',
      key: 'checkStatus',
      value: 'NO_PASS',
      permission: 'fund:check:pay:blend'
    },
    {
      name: '下载',
      type: 'download',
      key: 'checkStatus',
      value: 'PASS' + ',' + 'NO_PASS',
      permission: 'fund:check:pay:download'
    },
  ];
  ...
  lineBtnFn = {
    ...
    blend: (val, index, row) => {
      this.props.history.push({pathname: '/fundDefend/loanDetailInfo', state: {name: row.checkId}});
    },

    download: (val, index, row) => {
      loanDetailApi.checkPayDownload(row.checkId)
        .then((res) => {
  ...
  reissueFn = (checkId) => {
    loanDetailApi.checkPayAgain(checkId)
      .then((res) => {
        if (res.data.code === '200') {
          this.setState({
            loading: true,
  ...
  render() {
    return (
      <div>
        <SearchForm form={this.state.form} title='代付对账' onSubmit={(formValue) => this.onSubmit(formValue)}/>
