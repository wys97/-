...
import { Grid, Pagination, Table, Upload, Button } from "@alifd/next";
...
export default class DataTable extends Component {
  ...
  renderDetail = (value, index, record, path) => {
    return <Link to={{pathname: path + '/' + value, state: {name: value, row: record}}}>{value}</Link>;
  };
  ...
  loadColumns = (col, index) => {
    if (col.columns && col.columns.length > 0) {
      return <Table.ColumnGroup key={index} title={col.title} align='center'>
  ...
  render() {
    const renderOpt = (value, index, record) => {
      return this.props.lineBtn.map((item, i) => {
        ...
        return item.path ?
          <PermissionLink permission={item.permission} key={i} style={styles.lineBtn} to={item.path}
                          name={item.name}/> :
          <PermissionA permission={item.permission} key={i}
                       style={item.color ? {...styles.lineBtn, color: item.color} : styles.lineBtn} href="javascript:;"
        ...
        <Col l="24">
          {this.props.toolBtn.map((item, index) => {
            if (item.status === 'Upload') {
              return <Upload key={index} {...item.upload} style={{display: 'inline-block', marginRight: '10px'}}>
                <PermissionBtn permission={item.permission} type="primary" style={styles.button}
            ...
            return item.path ?
              <PermissionLink permission={item.permission} key={index} to={item.path} icon={item.icon}
                              name={item.name}/> :
              <PermissionBtn permission={item.permission} key={index} type="primary"
