...
export default class Tables extends Component {
  ...
  renderDetail = (value, index, record, path) => {
    ...
    if (record.approvalType == 'CREDIT') {
      
      let copyRecord = JSON.parse(JSON.stringify(record));
  
      if (copyRecord.currentLevel === "一级审批") {
        copyRecord.currentLevel = 1;
      } else if (copyRecord.currentLevel === "二级审批") {
        copyRecord.currentLevel = 2;
      } else {
        copyRecord.currentLevel = 3;
      }
      return (
        <Link
          to={{
            pathname: '/approvalManage/creditDetail/' + value,
            state: { name: value, row: copyRecord }
          }}
        >
          {value}
        </Link>
      );
    } else if (record.approvalType == 'DECREASE_INTEREST') {
      return (
        <Link
          to={{
            pathname: '/approvalManage/feeWaiverDetail/' + value,
    ...
    } else if (record.approvalType == 'LOAN') {

      return (
        <Link
          to={{
            pathname: '/approvalManage/loanDetail/' + value,
            state: { name: value, row: record }
          }}
        >
          {value}
      ...
      return (
        <Link
          to={{
            pathname: '/approvalManage/offlineRepayDetail/' + value,
            state: { name: value, row: record }
          }}
        >
          {value}
    ...
    //     CREDIT: "授信审批"
    // DECREASE_INTEREST: "息费调整审批"
    // LOAN: "贷款审批"
    // REPAY_OFFLINE: "线下还款登记审批"
  ...
  onChange = (selectedKeys, records) => {
    ...
    rowSelection.selectedRowKeys = selectedKeys   //用变量赋值， 用来刷新修改数据后，清空选中中的数组
    this.setState({
      rowSelection
    })
    this.props.selectedKey && this.props.selectedKey(rowSelection.selectedRowKeys);
    this.props.record && this.props.record(records);
  };


  componentWillReceiveProps(nextProps){
    if(nextProps.removeChecked){
      const{ rowSelection } = this.state;
